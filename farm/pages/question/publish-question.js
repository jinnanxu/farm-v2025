...
Page({
  ...
  bindTitleInput: function(e){
    this.setData({ 'question.questionContent': e.detail.value })
  },
  ...
  publishQuestion: function(){
    ...
    var req = this.data.question
    let picStr = this.data.uploadImgs.join(",")
    req.pics=picStr
    wx.request({
      url: app.globalData.apiBaseUrl+'/question/save',
      header: {
        ...
        if(res.data.code == "0"){
          wx.showModal({
            ...
            success (res2) {
              if (res2.confirm) {
                wx.switchTab({
                  url: '/pages/question/question-list',
                })
              }
            }
          })
